function BGE_Vm_Linux {
    ...
    do {
        ...
        switch ($input) {
            '1' {
                $global:TemplateLocation = "$global:template\$global:Ubuntu"
                $global:os = "ubuntu"
            } '2' {
    ...
    do {
        try {
            [ValidatePattern('[A-Z]')]$linuxuser = Read-Host "Enter Linux username / Enter for Sensa"
        }
      catch { }
    } until ($?)
    ...
    do {
        try {
            [ValidatePattern('ssh-rsa')]$sshkey = Read-Host "Enter your ssh key for user $linuxuser / cant skip this part" 
        }
        catch { }
    } until ($?)
    ...
    do {
        ...
        Switch ($IP) {
            S {
                ...
                do {
                    $subnetmask = Read-Host 'Enter subnetmask like for C mask 255.255.255.0'; if ($($addr = $null; [ipaddress]::TryParse($subnetmask, [ref]$addr) -and $addr.AddressFamily -eq 'InterNetwork')) { 'Valid IPv4 address' } else { 'Not valid IPv4 address' }
                }until ($addr.AddressFamily -eq 'InterNetwork')
                do {
                    try {
        ...
        switch ($input) {
            'C' {
                If ($DHCP -eq 'true') {
                    ...
                    $DNSDomain = "1"
                    $DHCP = "true"
                    BGE_create_vm_L $Name $CpuCount $RAM $net $IpAddress $subnetmask $DefaultGW $DNSServer $DNSDomain $DHCP $sshkey $linuxuser
                }
                Else {
                    BGE_create_vm_L $name $CpuCount $RAM $net $IpAddress $subnetmask $DefaultGW $DNSServer $DNSDomain $DHCP $sshkey $linuxuser
                }
