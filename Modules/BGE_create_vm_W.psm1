function BGE_Create_vm_W {
    ...
    $AdminPassword = [System.Web.Security.Membership]::GeneratePassword(12,0)
    ...
    $Cred = New-Object -TypeName "System.Management.Automation.PSCredential" -ArgumentList $winuser,$PWord

    
    #Invoke-Command -VMName $name -Credential $Cred -ScriptBlock {$u = Start-WUScan; Install-WUUpdates -Updates $u ; if (Get-WUIsPendingReboot) {restart-computer} }
    

    Write-Host "Done creating Vm [$name] With user [$winuser] password [$AdminPassword] Ipaddress [$output] Machine is running Windows update"
    Read-Host "Copy info and Press Enter to Continue"
}
